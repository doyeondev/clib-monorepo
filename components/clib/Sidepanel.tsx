...
interface SidePanelProps {
  ...
  clickedItem: any // getBack
  showSidebar: boolean
  setShowSidebar: (showSidebar: boolean) => void
}

export default function SidePanel({ data, clickedItem, showSidebar, setShowSidebar }: SidePanelProps) {
  const [selectedClauses, setSelectedClauses] = useState<number[]>([]) // 선택된 메모의 인덱스. 디폴트는 첫번째 메모로 지정함.
  ...
  useEffect(() => {
    ...
    // docsFile.nodeName // <- error!

    if (docsFile !== null && googleDocs !== null) {
      googleDocs.onload = function () {
  ...
  function clauseClickHandler(e: React.MouseEvent<HTMLDivElement>) {
    ...
    let selectedIdx = parseInt(e.currentTarget.id)
