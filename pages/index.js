      ...
      for (let j = 0; j < contractList[i].contentArray.length; j++) {
        ...
        if (match.length > 0 && titleLength === 0) {
          ...
          fullData = {
            ...
            clientName: contractList[i]?.clientName,
            ...
            <div className="flex w-full grow flex-col space-y-4 border-gray-300 text-sm">
              ...
              <div onClick={(e) => setSidebarData(item.article, item)} className="group flex cursor-pointer flex-col">
                <div className="mb-4 flex items-center justify-between">
                  <div className="grow text-base font-bold tracking-wide text-gray-700 hover:text-gray-800 group-hover:underline">{item.contractName}</div>
                  <div className="rounded bg-slate-100 px-1 py-0.5 text-xs text-gray-500 group-hover:visible">{item.clientName}</div>
                </div>
      ...
      <div className="flex flex-col rounded rounded-b-lg bg-white shadow">
        {searchResult.map((resultObj, index) => {
          ...
          if (searchType === 'contract') {
            ...
            additionalInfo = `<p>${resultObj.clientName}</p>`
            return (
            ...
            additionalInfo = `<p>${resultObj.source.title} (${resultObj.source.clientName})</p>`
            return (
              <Link
                key={index}
                href={{
...
const DashboardFooter = ({ onClickHandler, currentIndex, maxIndex }) => {
  return (
    <>
      <div className="mx-auto mt-4 flex items-center justify-between py-6">
        <button
          id="btnPrevious"
          name="paginationBtn"
