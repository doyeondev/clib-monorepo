...
const Contract = () => {
  ...
  const updateCategory = (e) => {
    if (e.target.id === 'clippedList') {
      //
      setContractList(contractAsset.filter((x) => clippedContract.includes(x._id)))
...
const ContractList = ({ contractList, currentIndex, setCurrentIndex, maxIndex, data, setData, setShowSidebar, showSidebar }) => {
  const { clippedContract, onClipClick, toastDetail, toastState, setToastState } = useContext(SessionContext)
                ...
                <div className="flex flex-col">
                  <div className="mb-4 flex items-center justify-between">
                          ...
                          {item.categories.length > 0 && (
                            <>
                              ...
                              {item.categories.map((category) => {
                                // let colorTheme = `bg-${category.color}-100`
                                // console.log('item', item)
                    ...
                    <Tooltip
                      html={
                        toastDetail.action === '추가' ? (
                          ...
                          <div className="flex h-full w-full items-center space-x-2 px-1">
                            ...
                            {/* <div>{`${toastDetail.action === '추가' ? `즐겨찾기에 ${toastDetail.action}되었습니다.` : `즐겨찾기에서 ${toastDetail.action}되었습니다.`}`}</div> */}
                      ...
                      theme="light"
                      // {}
                      // theme={toastDetail.action === '삭제' && 'light'}
                      // theme={`${toastDetail.action === '삭제' && 'light'} `}
                      // animation="scale"
                    >
              ...
              <div key={index} id={resultObj._id} onClick={(e) => setSidebarData(resultObj)} className="flex cursor-pointer items-center justify-between px-5 py-2.5 text-sm text-gray-700 hover:bg-gray-100">
                ...
                <div className="ml-4 flex items-center space-x-2 text-xs">
                  {resultObj.categories.length > 0 && (
                    <>
                      {/* <p className="text-gray-900">관련분야</p> */}
                      {resultObj.categories.map((category) => {
                        let colorTheme = `bg-${category.color}-100`
                        // console.log('item', item)
                        return (
                          <div key={category._id} className={`rounded ${colorTheme} px-1.5 py-0.5 text-gray-700`}>
                            {/* <div key={category._id} className={`rounded bg-stone-100 px-1.5 py-0.5 text-gray-700`}> */}
                            {category.title}
                          </div>
                        )
