...
import React, { FC, useState, useEffect, useContext, createContext, MouseEvent } from 'react'
...
import { getClibDataset, getClauseCategoryList } from '@/pages/api/clib'
...
const ClipContext = createContext<any>([])

const Clause = () => {
  const [contractList, setContractList] = useState<any[]>([])
  const [clauseList, setClauseList] = useState<any[]>([])
  const [categoryList, setCategoryList] = useState<any[]>([])
  const [categoryHolder, setCategoryHolder] = useState<any[]>([])
  ...
  const updateCategory = (e: MouseEvent) => {
    ...
    if (clickedItem === 'clippedList') {
      if (currentCategory === 'clippedList') {
        setClickedCategory((current: any) => [...current.filter((x: any) => x === 'allClauses')]) // '전체'눌렀으면 다른거는 다 꺼주고 '전체'만 눌러줌
      } else {
      ...
      if (type === 'search') {
        ...
        if (!clickedCategory.includes(clickedItem)) {
          ...
          if (clickedCategory.includes('allClauses')) {
            setClickedCategory((current: any) => [...current.filter((x: any) => x !== 'allClauses'), clickedItem]) // 전체는 빼주고 클릭된거 넣어줌
          } else if (clickedCategory.includes('clippedList')) {
            setClickedCategory((current: any) => [...current.filter((x: any) => x !== 'clippedList'), clickedItem]) // '전체'눌렀으면 다른거는 다 꺼주고 '전체'만 눌러줌
          }
          // "전체선택이 안 눌려있는 상태"
          else {
            if (clickedItem === 'allClauses') {
              ...
              // "전체선택 이외의 버튼이 눌린 상황"
              setClickedCategory((current: any) => [...current, clickedItem])
            }
          }
        }
        // 2. 빼주고
          ...
          setClickedCategory((current: any) => [...current.filter((x: any) => x !== clickedItem)])
        }
      }
    }
  }
  ...
  useEffect(() => {
    ...
    if (clickedCategory.includes('clippedList')) {
      ...
      if (clickedCategory.length < 1) {
        setClickedCategory((current: any) => [...current, 'allClauses']) // 하나도 안눌리면 자동으로 전체 보여줌
  ...
  return (
    <Layout>
      ...
      <CategoryContext.Provider value={{ contractList, clauseList, categoryList, currentCategory, updateCategory, clickedCategory }}>
        ...
        {/* <MainLayout contractList={contractList} /> : <Spinner /> */}
      ...
      <main className="mx-auto flex px-[10vw] py-6">
        ...
        <div className="flex w-[820px] flex-col">
          ...
          {_.orderBy(contractList, ['clause_category', 'idx'], ['asc', 'asc']).map((item, index) => {
            ...
            const category = categoryList.filter((x: any) => x._id === item.clause_category)[0]
            console.log('item', item)
            console.log('category', _.orderBy(contractList, ['clause_category', 'idx'], ['asc', 'asc']))
            const colors = require('tailwindcss/colors')
            let categoryColor
            if (clickedCategory.includes('allClauses')) {
              categoryColor = categoryList.filter((x: any) => x._id === item.clause_category)[0]?.color
              console.log('categoryList.filter((x) => x._id === item.clause_category)[0]', categoryList.filter((x: any) => x._id === item.clause_category)[0])
...
const SearchWrapper = ({
  ...
  return (
    <section className="mt-6 flex flex-col px-[10vw] py-4">
      ...
      <SearchInput searchType={searchType} setSearchType={setSearchType} setData={setData} setShowSidebar={setShowSidebar} showSidebar={showSidebar} />
    </section>
  )
}
...
const SearchInput: React.FC<{
  ...
  setShowSidebar: React.Dispatch<React.SetStateAction<boolean>>
  showSidebar: boolean
}> = ({ searchType, setSearchType, setData, setShowSidebar, showSidebar }) => {
  ...
  function runSearch(term: string) {
    let match = clauseList.filter((x: any) => x.title_ko?.match(getRegExp(term)) !== null)
  ...
  function runClauseSearch(term: string) {
    ...
    for (let i = 0; i < clauseList.length; i++) {
      ...
      let match = clauseList[i].title_ko.map((x: any) => x.title_ko?.match(getRegExp(term)) !== null)
      console.log('match', match)
      if (match.length > 0) {
    ...
    return (
      <div className="flex w-[458px] flex-col rounded rounded-b-lg bg-white shadow">
        {searchResult.map((resultObj, index) => {
          ...
          let category = categoryList.filter((x: any) => x._id === resultObj.clause_category)[0]
