...
export default function Result({ data }: ResultProps) {
  ...
  const passedData: any = router.query
          ...
          <div className="mx-auto w-[960px] px-[10vw] py-6">
            {data['contentArray']?.map((clause, idx) => {
              if (selectedClauses.includes(idx) === true) {
                const article = clause.filter((x) => x.tag === 'h2')[0]
...
export const getServerSideProps = async ({ query }: { query: { _id: string } }) => {
